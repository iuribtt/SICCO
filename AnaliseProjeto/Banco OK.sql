...
CREATE TABLE IF NOT EXISTS `mydb`.`tb_modelo` (
  ...
  INDEX `fk_tb_modelo_tb_Marca1_idx` (`tb_Marca_id_marca` ASC),
  CONSTRAINT `fk_tb_modelo_tb_Marca1`
    FOREIGN KEY (`tb_Marca_id_marca`)
    REFERENCES `mydb`.`tb_Marca` (`id_marca`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`tb_veiculo` (
  ...
  INDEX `fk_tb_veiculo_tb_modelo1_idx` (`tb_modelo_id_modelo` ASC),
  ...
  CONSTRAINT `fk_tb_veiculo_tb_modelo1`
    ...
    REFERENCES `mydb`.`tb_modelo` (`id_modelo`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`tb_funcionario` (
  ...
  INDEX `fk_tb_funcionario_tb_cliente1_idx` (`tb_cliente_CPF_pessoa` ASC),
  CONSTRAINT `fk_tb_funcionario_tb_concessionaria1`
    FOREIGN KEY (`tb_concessionaria_id_concessionaria`)
    REFERENCES `mydb`.`tb_concessionaria` (`id_concessionaria`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tb_funcionario_tb_cliente1`
...
CREATE TABLE IF NOT EXISTS `mydb`.`tb_negociacao` (
  ...
  INDEX `fk_tb_negociacao_tb_cliente1_idx` (`tb_cliente_CPF_pessoa` ASC),
...
CREATE TABLE IF NOT EXISTS `mydb`.`Tb_compra` (
  ...
  PRIMARY KEY (`cod_compra`, `tb_negociacao_cod_negociacao`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`tb_venda` (
  ...
  `tb_negociacao_cod_negociacao` INT NOT NULL,
  PRIMARY KEY (`cod_venda`, `tb_negociacao_cod_negociacao`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`tb_cliente_has_tb_concessionaria` (
  `tb_concessionaria_id_concessionaria` INT NOT NULL,
  `tb_cliente_id_pessoa` INT NOT NULL,
  PRIMARY KEY (`tb_concessionaria_id_concessionaria`),
  INDEX `fk_tb_cliente_has_tb_concessionaria_tb_concessionaria1_idx` (`tb_concessionaria_id_concessionaria` ASC),
  INDEX `fk_tb_cliente_has_tb_concessionaria_tb_cliente1_idx` (`tb_cliente_id_pessoa` ASC),
  ...
  CONSTRAINT `fk_tb_cliente_has_tb_concessionaria_tb_cliente1`
    FOREIGN KEY (`tb_cliente_id_pessoa`)
    REFERENCES `mydb`.`tb_cliente` (`id_pessoa`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
